...
建議：把這幾張字卡的 `concept` 更新成跟目前 prompt 一致的描述（Studio 的編輯字卡就能改）。注意 `concept` 會算進 prompt 的 `contextHash`，改了之後這幾個字現有的 prompt 會被標示「需要更新」。影片本身不受影響；口、米、木這三支影片的字形也都正確，所以不急，可以等下次要重做這幾個字時再一起改。
